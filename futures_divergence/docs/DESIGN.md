...
> 상태: 1단계 설계 + 2단계(데이터 계층·RSI·피벗) 진행. 3단계부터는 승인 후 진행.
...
- **동률 규칙** (`pivot.tie_rule`):
  - `strict`(기본): 오른쪽도 엄격히 `<`. 이웃에 같은 값이 있으면 피벗이 아니다. 요구사항 문구 "모두 낮은" 그대로.
  - `first`: 오른쪽은 `≤` 허용 → 같은 저가가 이어지면 **첫 봉**만 피벗.
...
| `...SETTLED` 심볼 | 폐지·재상장 때 생기는 별도 시리즈. 같은 코인으로 합치되 겹치는 구간은 체결이 있는 쪽을 쓴다 (2단계 점검) |
...
1. **피벗 동률 규칙**: 기본 `strict`(요구사항 문구). 2단계 측정: `first` 대비 빠지는 피벗은 15m 0.9~3.6%, 1h 0.3~1.5%,
   4h 0.6% 이하, 1d 0.2% 이하 (BTC·ETH·SOL·DOGE·1000PEPE, 2024-01~2026-09).
...
10. **펀딩비**: 아카이브는 월 파일만 있어 이번 달(진행 중) 펀딩비가 없다 → 백테스트 마지막 달은 REST(국내 PC)로
    보충하거나 0 으로 둔다(보고서에 표시).

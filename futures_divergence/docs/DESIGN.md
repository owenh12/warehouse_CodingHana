...
| 진입 | TF 마다 독립 (다중 TF 동시 성립 조건은 시험 후 원복, `confluence.enabled: false` 선택 규칙으로 남김) | 사용자 결정 (4단계 후) |
| 손절 (전략 수정) | 진입가 ∓ 2.5 × ATR(신호 TF). 손절폭 > 진입가 × 0.5% 여야 진입 | 사용자 결정 (4단계 후) |
| MDD 킬 스위치 | 제거 (`kill_switch.enabled: false`). 리스크 규칙은 일일 손실 3% 만 | 사용자 결정 (4단계 후) |
...
- **진입 조건**: ① 신호 시각 순위 ≤ 10 ② 손절폭(ATR×2.5) > 진입가 × 0.5%. TF 마다 독립 판정.
- (선택 규칙, 기본 off) 다중 TF 동시 성립: 같은 코인·같은 방향으로 그 시각에 유효한 **서로 다른 TF 신호 ≥ 2개**
  (`confluence`; 신호는 확정 후 자기 TF `validity_bars`=3 봉 동안 유효, 같은 TF 여러 개는 1개로 셈).
  판정은 새 신호가 확정될 때만 하고(= 확정 신호), 그때 유효한 다른 TF 신호는 이전 진입에 쓰였어도 다시 센다.
  손절·시간 청산의 ATR·봉과 우선순위의 TF 는 확정 신호의 TF 다. 스킵 사유 `no_confluence`, `skipped_stop_too_tight`.
...
| ATR | 확정 신호 TF 의 Wilder ATR(14), t3(p3) 봉 값 |
| 손절 | **Long `진입가 − ATR×2.5`, Short `진입가 + ATR×2.5`** (전략 수정 후, `stop.basis: entry_price`; 이전 규칙 `signal_bar` = Low(t3)/High(p3) 기준). 스탑마켓(테이커). 진입가 = 실제 체결가 |
| 최소 손절폭 | 손절폭(ATR×2.5) > 진입가 × 0.5% 여야 진입 (`stop.min_distance_pct`). 주문 전 판정이라 신호 봉 종가를 진입가로 쓴다 |
...
| 시간 청산 | 진입 후 확정 신호 TF 60봉 안에 목표 미도달 → 60봉째 마감 후 다음 15분봉 시가 시장가 |
| 같은 봉 손절·익절 | 손절 우선 |
...
| ~~누적 MDD 30%~~ (제거, 사용자 결정) | 운용 시작 후 최고 평가금액 대비 | 신규 진입 차단 + `flatten_all` 또는 `keep_stops` | 발동 ID 를 `release_ack` 에 입력 (자동 재개 금지) |

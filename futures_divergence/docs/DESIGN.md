...
| 진입 (전략 수정) | 같은 코인·방향으로 서로 다른 TF 신호 2개 이상이 유효할 때만. 신호는 자기 TF 3봉 동안 유효, 판정은 새 신호 확정 때 | 사용자 결정 (4단계 후) |
| 손절 (전략 수정) | 진입가 ∓ 2.5 × ATR(확정 신호 TF). 손절폭 > 진입가 × 0.5% 여야 진입 | 사용자 결정 (4단계 후) |
...
- **진입 조건 (전략 수정 후)**: ① 신호 시각 순위 ≤ 10 ② 같은 코인·같은 방향으로 그 시각에 유효한 **서로 다른 TF 신호 ≥ 2개**
  (`confluence`; 신호는 확정 후 자기 TF `validity_bars`=3 봉 동안 유효, 같은 TF 여러 개는 1개로 셈) ③ 손절폭 > 진입가 × 0.5%.

...
> 상태: 1·2·3단계 완료 ([2단계](STAGE2_DATA_REPORT.md) · [3단계](STAGE3_SIGNALS_REPORT.md) 보고서). 4단계는 승인 후 진행.
...
선택 규칙 `structure.anchor_must_be_extreme` (기본 false = 요구사항 그대로): 켜면 판정 4 에 `Low(t1) = min Low[t1..p2]`
(약세 `High(p1) = max High[p1..t2]`) 를 더한다. p2 가 바뀌면 새 p2 기준으로 다시 계산한다.
...
10. **백테스트 끝 = 지난달 말** (`backtest_period.end: last_month_end`, 사용자 결정): 아카이브 펀딩비는 월 파일만 있어
    진행 중인 달을 뺀다.
11. **t1(p1) 극값 여부** (3단계 확인 대기): 요구사항에는 t1 이 [t1, p2] 의 최저가여야 한다는 조건이 없다. BTC 15분봉
    신호의 절반가량에서 t1 과 p2 사이에 더 낮은 저가가 있다. `structure.anchor_must_be_extreme` 로 선택 (기본 false).

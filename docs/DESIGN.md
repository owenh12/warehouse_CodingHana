...
> 상태: 1단계 완료. **2단계는 바이낸스만 진행 중**이다(사용자 지시). KIS·KOSPI200·환율은 보류.
> 2단계 산출물은 §9와 §13에 있다.
...
| 코인 과거 데이터 출처 | ccxt REST(기본) + data.binance.vision 아카이브(선택) | 아카이브는 SHA-256 검증, 대량 수집, REST 교차검증, 지역 차단 대안 |
...
| 가상화폐 15분봉 | `history_source: rest`(ccxt) 또는 `vision`(data.binance.vision 월·일별 zip) | 진행 중인 봉은 제외. 아카이브 현물 CSV의 마이크로초 시각(2025~)·선물 CSV 헤더 자동 처리 |
| 펀딩비 | 바이낸스 fapi 펀딩 이력 (REST) 또는 월별 아카이브 | 선물 선택 시에만 사용 |
...
3. **바이낸스 접근.** 이 원격 환경의 네트워크 정책이 api.binance.com, fapi.binance.com, data-api.binance.vision, data.binance.vision을 모두 차단(프록시 403)했다(2026-09-25 확인). 허용하더라도 api.binance.com은 일부 지역(예: 미국 IP)에서 451로 차단될 수 있다. 그 경우 `history_source: vision` 또는 `spot_public_api: https://data-api.binance.vision/api/v3`를 쓴다. 국내 거주자의 해외 거래소 이용과 관련된 규제·계정 요건은 사용자가 확인할 사항이다.
...
| 모듈 | 내용 |
|---|---|
| `data/binance.py` | ccxt REST 공급자: 페이지네이션, 진행 중 봉 제외, 지수 백오프 재시도(`RetryPolicy`), 펀딩비, 주문 필터 파싱 |
